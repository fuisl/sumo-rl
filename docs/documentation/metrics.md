...
| `nash-average-speed` | `exp(mean(log(phase_average_speed + 0.1)))` | per-phase mean normalized speed ratio over the incoming lanes served by each green phase; empty phases use `phase_average_speed = 1.0` | Higher is better; the geometric mean penalizes slow phases |
| `weighted-nash-average-speed` | `exp(sum(phase_weight * log(phase_average_speed + 0.1)))` | same phase-average-speed term as `nash-average-speed`, plus `phase_weight = phase_max_waiting_time / sum(phase_max_waiting_time)` with `phase_max_waiting_time = max(vehicle waiting time)` for that phase; empty phases use `phase_max_waiting_time = 0`, and zero total max waiting falls back to uniform weights | Higher is better; the weighted geometric mean emphasizes phases with the largest max waiting |
