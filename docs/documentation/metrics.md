...
| `nash-average-speed` | `exp(mean(log(window_mean_phase_average_speed + reward_nash_epsilon)))` | per-phase mean normalized speed ratio over a rolling per-signal NSW window; the window is `ceil(fixed_time_cycle_length * reward_nsw_window_cycle_multiplier)` SUMO seconds, where fixed-time cycle length is derived from that signal's SUMO program durations; empty phases use `phase_average_speed = 1.0`; `reward_nash_epsilon` defaults to `0.1` and `reward_nsw_window_cycle_multiplier` defaults to `1.0` | Higher is better; the geometric mean penalizes phases that stay slow over the window |
| `weighted-nash-average-speed` | `exp(sum(window_phase_weight * log(window_mean_phase_average_speed + reward_nash_epsilon)))` | same windowed phase-average-speed term as `nash-average-speed`, plus `window_phase_weight = window_max_phase_waiting_time / sum(window_max_phase_waiting_time)` using each phase's max waiting time observed over the rolling window; empty phases use `window_max_phase_waiting_time = 0`, and zero total max waiting falls back to uniform weights | Higher is better; the weighted geometric mean emphasizes phases with the largest waiting-time spike over the window |
| `vehicle-weighted-nash-average-speed` | `exp(sum(window_phase_vehicle_weight * log(window_mean_phase_average_speed + reward_nash_epsilon)))` | same windowed phase-average-speed term as `nash-average-speed`, plus `window_phase_vehicle_weight = window_mean_phase_vehicle_count / sum(window_mean_phase_vehicle_count)` using each phase's mean vehicle count observed over the rolling window; empty phases use `window_mean_phase_vehicle_count = 0`, and zero total vehicle count falls back to uniform weights | Higher is better; the weighted geometric mean emphasizes phases with the most vehicles over the window |
