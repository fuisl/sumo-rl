"""FGS SAC learner with separate CTDE critic and actor Q contexts."""
...
from sumo_rl.agents.fgs.rllib_module import FGS_ACTOR_QF_PREDS, FGS_ACTOR_QF_TWIN_PREDS


class FGSSACTorchLearner(SACTorchLearner):
    """Discrete SAC loss that uses replay joint actions for critic TD only."""
    ...
    def _compute_loss_for_module_discrete(
        ...
        *,
        ...
        actor_q = fwd_out.get(FGS_ACTOR_QF_PREDS, fwd_out[QF_PREDS])
        if config.twin_q:
            actor_q_twin = fwd_out.get(FGS_ACTOR_QF_TWIN_PREDS, fwd_out[QF_TWIN_PREDS])
        ...
        self.metrics.log_dict(
            {
                ...
                "fgs_actor_q_mean": torch.mean(actor_q),
