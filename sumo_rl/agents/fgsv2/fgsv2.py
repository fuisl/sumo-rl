"""FGSv2-specific RLlib config, training loop, and metrics."""
...
from sumo_rl.agents.fgsv2.learner import FGSv2SACTorchLearner
from sumo_rl.agents.fgsv2.rllib_module import (
    build_fgsv2_sac_module_spec,
    build_fgsv2_sac_multi_module_spec,
    normalize_fgsv2_model_config,
)
...
def _build_fgsv2_context(cfg: Any, run_dir: Path, params: Dict[str, Any]) -> RllibAlgorithmContext:
    ...
    model_config = _fgsv2_model_config(params)
    sample_env = build_fgsv2_parallel_env(
        cfg,
        run_dir,
        model_config,
    ...
    def _creator(env_config):
        ...
        return ParallelPettingZooEnv(build_fgsv2_parallel_env(cfg, run_dir, model_config, seed=seed))
...
def build_config(cfg: Any, run_dir: Path):
    ...
    if int(params.get("n_step", 1)) != 1:
        raise ValueError("FGSv2 central_graph_joint_action critic requires algorithm.params.n_step=1.")
    ...
    params["model_config"] = _fgsv2_model_config(params)
    ...
    context = _build_fgsv2_context(cfg, run_dir, params)
    ...
    rl_module_specs = {
        policy_id: build_fgsv2_sac_module_spec(
    ...
    config = config.rl_module(
        rl_module_spec=build_fgsv2_sac_multi_module_spec(rl_module_specs, model_config=params["model_config"])
    )
    learners = getattr(config, "learners", None)
    if callable(learners):
        config = config.learners(learner_class=FGSv2SACTorchLearner)
    else:
        config = config.training(learner_class=FGSv2SACTorchLearner)
