...
def normalize_custom_sac_model_config(model_config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    top_level_custom_updates = {key: incoming.pop(key) for key in list(incoming) if key in DEFAULT_CUSTOM_SAC_MODEL_CONFIG}
    custom_updates = (
    ...
    actor_encoder_type = str(actor_encoder.get("type", "mlp") or "mlp").lower()
    critic_encoder_type = str(critic_encoder.get("type", "mlp") or "mlp").lower()
    if actor_encoder_type != "mlp":
        raise ValueError("custom SAC currently supports actor.encoder.type='mlp' only.")
    if critic_encoder_type != "mlp":
        raise ValueError("custom SAC currently supports critic.encoder.type='mlp' only.")
    ...
    if communication_type not in {"none", "identity", "message_passing", "gat"}:
        raise ValueError("custom SAC communication.type must be one of: none, identity, message_passing, gat.")
    ...
    class CustomSACCatalog(SACCatalog):
        ...
        def __init__(self, observation_space, action_space, model_config_dict, view_requirements=None):
            ...
            if critic_hidden:
                ...
                required_qf_output_dim = self.action_space.n if hasattr(self.action_space, "n") else 1
...
def build_custom_sac_module_class():
    ...
    class CustomSACCommunicationBlock(nn.Module):
        """Swappable latent hook for future communication operators."""
