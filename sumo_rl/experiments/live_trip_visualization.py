...
def run_best_checkpoint_trip_visualization(
    ...
    *,
    ...
    out_dir = (
        ...
        else ROOT / "experiments" / "artifacts" / "live_trip_viz" / run_path.name
    )
...
def _restore_evaluate_and_trace(
    ...
    *,
    ...
    if not hasattr(cfg, "logging") or cfg.logging is None:
        cfg.logging = {}
    cfg.logging.eval_use_libsumo = False
...
def _tls_positions_from_extraction(extraction: NodeExtraction) -> dict[str, list[float]]:
    positions = {}
...
def _edge_endpoints(root: ET.Element, positions: dict[str, Point]) -> dict[str, tuple[str, str]]:
    ...
    for edge in root.findall("edge"):
        ...
        target = edge.get("to")
        if edge_id and edge.get("function") != "internal" and source in positions and target in positions:
